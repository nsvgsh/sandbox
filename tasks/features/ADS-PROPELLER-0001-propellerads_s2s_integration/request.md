...
- Support Telegram start-parameter based attribution for campaigns that deep-link to the bot/mini app.
...
- Aligns with PropellerAds documentation and constraints for Telegram flows (64-char `start` param; allowed charset; token format).

## Scope
- Parse and store PropellerAds identifiers from Telegram `start` payload: `${SUBID}`, `{campaignid}`, `{zoneid}`, and a network tag (e.g., `prop`).

This article provides code examples in different programming languages on how a
developer could authorize a user using Telegram init data.

First of all, it is required to begin with the transmitting init data from the client side to the
server. We could do it using this code:

We are sending a request to an imaginary server using the https://example.com/api
URL. This request uses the HTTP POST method (you can use whatever you want) and
appends the Authorization header, which is the most important here. It represents a
string containing 2 parts divided by a space. The first one describes the authorization
method (in this case, our server is going to support several others), and the second one
contains authorization data. In the case of Telegram Mini Apps, the second part is raw init
data.
...
Now, when init data is transmitted to the server side, we should create a simple HTTP
server which uses this data and authorizes the user.
...
// We expect passing init data in the Authorization header in the following
// <auth-type> <auth-data>
```
```
typescript
...
// We consider init data sign valid for 1 hour from their creation
expiresIn: 3600 ,
});
...
```
```
// After the HTTP server was launched, try sending an HTTP GET request to the
// http://localhost:3000/ with an Authorization header containing data in the
```
```
package main
```
...
func withInitData(ctx context.Context, initData initdata.InitData) context.Co
...
// We expect passing init data in the Authorization header in the fol
// <auth-type> <auth-data>
// <auth-type> must be "tma", and <auth-data> is Telegram Mini Apps i
...
switch authType {
case "tma":
// Validate init data. We consider init data sign valid for 1 hou
// creation moment.
if err := initdata.Validate(authData, token, time.Hour); err != n
context.AbortWithStatusJSON( 401 , map[string]any{
"message": err.Error(),
})
return
